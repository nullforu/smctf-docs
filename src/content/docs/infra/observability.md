...
![image](images/observability/image-7.png)
...
SMCTF observability Helm 차트엔 아래와 같은 Prometheus 모니터링 관련 구성이 포함되어 있습니다. 특별한 경우가 아니라면 기본값을 권장하며, 필요 시 수정하여 사용할 수 있습니다.
...
Prometheus 스택 설치가 끝나면 `kubernetes-observability` 차트를 설치합니다.
...
설치 후 Prometheus 대시보드에서 타겟에 잘 올라오는지 확인하세요. 백엔드 애플리케이션과 Container Provisioner에 대한 `/metrics` 모니터링이 기본적으로 구성되어 있고 ServiceMonitor CRD가 기본적으로 Observability Helm 차트에 포함됩니다.

![image](images/observability/image.png)

![image](images/observability/image-1.png)
...
![image](images/observability/image-2.png)

![image](images/observability/image-3.png)
...
이와 관련된 observability Helm 차트 구성은 아래와 같습니다. 특별한 경우가 아니라면 기본값을 권장하며, 필요 시 수정하여 사용할 수 있습니다.
...
DaemonSet으로 직접 배포되기 때문에 별도의 FluentBit Helm 차트를 설치할 필요는 없습니다. FluentBit 또한 Observability Helm 차트에서 관리합니다.

정상적으로 설치가 되었다면 아래와 같이 CloudWatch Logs에서 로그 그룹과 로그 스트림이 생성되고 로그가 수집되는 것을 확인할 수 있습니다.

![image](images/observability/image-4.png)

![image](images/observability/image-5.png)

또한 Live Tail을 통해 실시간으로 로그가 수집되는 것을 확인할 수도 있습니다.

![image](images/observability/image-6.png)
