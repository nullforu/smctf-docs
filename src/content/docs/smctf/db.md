...
SMCTF에선 기본적으로 관리자 계정을 제공하지 않으며, 직접 DB에 접근하여 관리자 계정을 생성했어야 합니다. 이러한 불편함은 [`v1.2.0`](/changelog)에서 추가된 관리자 팀/계정 부트스트랩 기능을 통해 개선되었습니다.
...
위 환경 변수에서 `BOOTSTRAP_ADMIN_TEAM`과 `BOOTSTRAP_ADMIN_USER`를 `true`로 설정하고 관리자 이메일 및 비밀번호를 설정하면 애플리케이션이 시작될 때 자동으로 Admin 팀과 초기 관리자 계정이 생성됩니다.
(이때 이메일 및 비밀번호가 설정되어 있지 않으면 관리자 계정이 생성되지 않습니다.)

이때 DB에 아무 데이터가 없는 상태에서만 관리자 팀과 계정이 생성됩니다. 스냅샷된 DB 등이 아니라면 일반적으로 DB에 데이터가 없는 상태로 프로비저닝되므로 초기 관리자 팀과 계정이 생성됩니다.

팀 이름은 무조건 `Admin`으로 고정됩니다. 관리자 계정은 `Admin` 역할이 할당되며 관리자 권한이 필요한 API에 접근할 수 있습니다.
관리자는 점수 및 스코어보드 집계에서 제외되며 프론트엔드에서 숨겨집니다.
