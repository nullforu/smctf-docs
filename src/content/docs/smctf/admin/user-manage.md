...
![image](images/user-manage/image.png)
...
제한(차단) 시 받는 불이익에 대해선 [유저](/smctf/users) 문서를 참고하세요. 제한(차단) 시 마땅한 차단 사유를 입력해야 합니다. 차단 사유는 유저가 웹 페이지에 접속했을 때 표시됩니다.

![image](images/user-manage/image-1.png)

차단 시 해당 유저의 역할은 `Blocked`로 변경되며 차단 사유 및 차단 일시가 기록됩니다. 관리자(Admin)는 차단할 수 없습니다.

![image](images/user-manage/image-2.png)
