...
<Aside type='caution'>Deprecated. 해당 문서에 대해선 더 이상 관련된 업데이트가 이루어지지 않을 예정이며, [인프라](/infra) 구조가 아닌 단일 애플리케이션으로써의 SMCTF 설치 및 Dockerfile 구성에 대해선 권장되지 않습니다.</Aside>
...
이 문서는 로컬에서 테스트 용도로 SMCTF를 설치하고 구성하는 방법을 설명합니다. 실제 배포 환경에서는 EKS Kubernetes 클러스터와 같은 컨테이너 오케스트레이션 플랫폼을 사용하며, 자세한 내용은 [인프라 문서](/infra)에서 설명합니다.
